...
| `senate_ptr` | **4 consecutive**, last OK 2026-08-02T13:30 | **SETTLED 2026-08-06T13:30:44Z: blocked from Worker egress. `Error: efd home 403`.** The failure is the FIRST request of the handshake, `GET /search/home/` — not the agreement step, not the search. Same minute, same declared User-Agent, same URL from a residential client: **200, three for three.** That asymmetry is the whole finding, and it is the same family as `press_cftc_enforcement` 403, `treasury_auction` 525 and `rate_boe` 500. | **PARKED, needs a courier route.** Whether GitHub-runner egress reaches efdsearch.senate.gov is UNVERIFIED and must be measured before any courier work (D-25: a laptop probe proves nothing about runner egress either). | 2026-08-06 |
