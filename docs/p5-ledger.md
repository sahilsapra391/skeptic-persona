...
| D-55 | 2026-08-06 | **GitHub Actions runners are unavailable for this repo, intermittently and then persistently.** Jobs are cancelled at exactly 15:00 with `steps: []` — the queue timeout, never assigned a runner. Confirmed on four runs across `main` and PR #175 (jobs 92685027225, 92688138780, 92692177662 and two on main). One run DID get a runner at 17:08 and passed in 3m3s, so it is capacity rather than a total outage. Main itself is healthy: typecheck clean and 1,184 passing locally on the same commit. **Not diagnosable from here** — `gh api .../settings/billing/actions` needs the `user` scope, and expanding auth scope is not mine to do. Private-repo Actions minutes are account-wide and the relay workflow's own header flags them as a shared constraint. **Parked at blocked-owner**; nothing is force-merged past a check, which is ruling 5 working as intended. | blocked-owner |
