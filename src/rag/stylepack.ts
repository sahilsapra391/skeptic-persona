...
//
// HARD BOUNDARY: no third-party text. Every example below is synthetic,
// written for this file in the desk's own voice. The corpus informed the
// PATTERNS (and the measured numbers in the congress section); no sentence
// from it appears here, and an offline 8-gram sweep against the corpus is
// part of the PR checklist for any edit to this file.
//
...
REGISTER
Wire-terse. Median post 100-140 weighted characters. No hashtags. No
...
/**
 * Positive examples are the ONLY complete post-shaped text in this file, and
 * every one is register-tested (test/stylepack.test.ts): each must pass
 * checkRegister and fit the weighted 280. Synthetic — the numbers are
 * invented for illustration and marked as such in the prompt assembly, so
 * they can never be mistaken for payload facts.
 */
export const MOVE_EXAMPLES: readonly StyleExample[] = [
  ...
  // Juxtaposition: two parsed facts, adjacent, zero connective tissue.
  {
...
1. COMPRESSION. Subject, verb, number, stop. No adjectives, no hedges, no
   ...
   or its attribution, it goes. The best wire posts in the studied corpus run
   ~95-125 characters; ours target the same band.
...
export const ANTI_PATTERNS: readonly AntiPattern[] = [
  ...
  },
  {
    ...
    why: "80% of one studied account's posts are meaningless without the attached image. Wire posts get screenshotted as evidence; the text must stand alone.",
  },
];
...
Of 175 congress-tagged competitor posts under 280 chars, only 13 are actual
trade-disclosure posts, and every one follows a single shape: alarm opener,
editorial frame line, then member + verb + amount + ticker + date. Our lane is
that final clause WITHOUT the first two: the disclosure mechanics ARE the
story.
...
- 52% of studied congress posts open ALL-CAPS and 40% open BREAKING. Zero of
...
export function stylePackFor(archetype: ArchetypeId): string {
  const sections = [
    VOICE_CORE,
    MOVES,
    "EXAMPLES OF THE MOVES (synthetic — the numbers are illustrations, not facts):",
    ...MOVE_EXAMPLES.map((e) => `---\n${e.text}\n---`),
