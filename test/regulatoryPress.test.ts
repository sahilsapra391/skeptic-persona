...
describe("enforcement wire (live fixtures)", () => {
  ...
  it("these feeds carry no categories, so everything is newsworthy by config", () => {
    ...
    expect(isNewsworthy(src, { title: "X v. Y", link: "https://sec.gov/x", publishedIso: "2026-07-27T00:00:00.000Z", categories: [], guid: "g", description: null, dateOnly: false })).toBe(true);
  });
});
